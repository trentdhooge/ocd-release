...
GetOptions('group|g=s' => \$groupname, 'id|i=s' => \$groupid, 'default|r=s' => \$default, 'directory|d=s' => \@directories, 'comps|c' => \$header, 'footer|f' => \$footer, 'xml|x' => \$fullxml, 'help|h' => \$needhelp, 'version|v' => \$version, 'type|t=s' => \$type, 'uservis|u=s' => \$uservisible);
...
# make sure that there is a type is defined
if($type eq '') {
    print STDERR "You must specify a yum type\n";
    print STDERR "(using the --type flag)\n";
    exit;
}
...
sub output_group {
    print "  <group>\n";
    print "   <id>$groupid</id>\n";
    print "   <uservisible>$uservisible</uservisible>\n";
    print "   <name>$groupname</name>\n";
    print "   <default>$default</default>\n";
    print "   <packagelist>\n";
    foreach(@pkgs) {
        print "     <packagereq type=\"$type\">$_</packagereq>\n";
    }
    print "   </packagelist>\n";
    print "  </group>\n";
}
